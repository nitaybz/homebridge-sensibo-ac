	...
	accessories: async function (callback) {
		...
		if (this.apiKey) {
			try {
				...
				this.cachedState = await storage.getItem('state')
			} catch(err) {
			...
			if (!this.processingState) {
				...
				setTimeout(async () => {
					try {
						...
						try {
							await storage.setItem('state', this.cachedState)
						} catch(err) {
		...
		if (this.apiKey) {
			...
			try {
				...
				await storage.setItem('devices', pods)

			} catch(err) {
				this.log('ERROR getting devices status from API!')
				const cachedPods = await storage.getItem('devices')
				if (cachedPods)
					pods = cachedPods
			}
		}
