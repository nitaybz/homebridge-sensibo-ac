...
- **Auto Detect Configurations** - Automatically detect your devices, their capabilities and add available control options to Home app (HomeKit). More details below
...
- **Occupancy Sensor** - get the Home/Away status from Sensibo API to Home app via Occupancy sensor
...
If your Sensibo app has **Vertical Swing** control, an "Oscillate" toggle will be added to the existing AC accessory in Home app to control it.
...
3. **Reset Filter Indication** - Stateless button (appears only in Eve app due to Apple Home app limitations) that resets the counter of the filter life. Normally you would click this button right after you cleaned the filters.
