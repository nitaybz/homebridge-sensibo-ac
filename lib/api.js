...
const getRequest = (path, queryString) => {
	return new Promise((resolve, reject) => {
		if (debug)
			log('making a request:', `${baseUrl+path}?apiKey=${apiKey}&${queryString}`)
		request({
			method: 'GET',
			uri: `${baseUrl+path}?apiKey=${apiKey}&${queryString}`
...
const postRequest = (path, json) => {
	return new Promise((resolve, reject) => {
		if (debug)
			log('making a request:', `${baseUrl+path}?apiKey=${apiKey}`, '\n', JSON.stringify(json, null, 2))
		request({
			method: 'POST',
			uri: `${baseUrl+path}?apiKey=${apiKey}`,
			json: json
		}, function (error, response, data) {
