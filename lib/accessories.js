...
module.exports = function (Accessory, Service, Characteristic, HomebridgeAPI, uuid, FakeGatoHistoryService) {
	...
	function acAccessory(config, state) {
		...
		this.log('!this.disableHistoryStorage', !this.disableHistoryStorage)
		if (this.enableHistoryStorage) {
			this.loggingService = new FakeGatoHistoryService('weather', this, { storage: 'fs', path: HomebridgeAPI.user.persistPath() + '/../plugin-persist' })
		}
	...
	acAccessory.prototype.getCurrentTemperature = function (callback) {
		this.refreshState()
		if (this.debug) t
			his.log('Getting Room Temperature', this.name)
		const temp = this.state.measurements.temperature
