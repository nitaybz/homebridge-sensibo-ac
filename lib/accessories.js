...
module.exports = function (Accessory, Service, Characteristic, uuid, fakegatoHistoryService) {
	...
	function acAccessory(config, state) {
		...
		this.disableHistoryStorage = config.disableHistoryStorage,
		this.log = config.log
		...
		if (!config.disableHistoryStorage)
	...
	acAccessory.prototype.getServices = function () {
		...
		if (this.enableSyncButton) {
			...
			this.SyncButtonService = new Service.Switch(this.name + ' Sync')
	...
	acAccessory.prototype.getTemperatureDisplayUnits = function (callback) {
		...
		callback(null, this.temperatureUnit === 'C' ? Characteristic.TemperatureDisplayUnits.CELSIUS : Characteristic.TemperatureDisplayUnits.FAHRENHEIT)
	}
	...
	acAccessory.prototype.updateHomeKit = function (acState, measurements) {
		...
		if (this.debug && !this.setProcessing && (acState || measurements)) {
