const Accessory, Service, Characteristic, uuid, SensiboAccessory
...
module.exports = function (Accessory, Service, Characteristic, uuid) {
	...
	function acAccessory(config, state) {
		...
		const id = uuid.generate('hbdev:sensibo:pod:' + this.id)
	...
	acAccessory.prototype.getServices = function () {
		...
		if (this.enableSyncButton) {
			...
			this.SyncButtonService = new Service.Switch(this.name + " Sync State")
	...
	acAccessory.prototype.setTargetHeatingCoolingState = function (state, callback) {
		let mode
		switch (state) {
			case Characteristic.TargetHeatingCoolingState.OFF:
				if (this.debug)
					this.log(this.name + ' -> Setting Target Thermostat State to OFF')
				this.setNewState('acPower', false)
				callback()
				return
	...
	acAccessory.prototype.getCoolingThresholdTemperature = function (callback) {
		this.refreshState()
		if (this.debug) this.log('Getting Cooling Threshold Temperature', this.name)
	
		const targetTemp = this.state.acState.targetTemperature
		const mode = this.state.acState.mode
		const on = this.state.acState.on

		if (on && (mode === 'cool' || mode === 'auto')) {
	...
	acAccessory.prototype.getCurrentRelativeHumidity = function (callback) {
		this.refreshState()
		if (this.debug) this.log('Getting Current Relative Humidity', this.name)
	...
	acAccessory.prototype.getFanSwing = function (callback) {
		this.refreshState()
		if (this.debug) this.log('Getting AC Swing State', this.name)
	...
	acAccessory.prototype.setACRotationSpeed = function (speed, callback) {
		...
		speed = HKtoFanLevel(state, (this.capabilities.cool || this.capabilities.heat || this.capabilities.auto).fanLevels)
	...
	acAccessory.prototype.setFanRotationSpeed = function (speed, callback) {
		...
		speed = HKtoFanLevel(state, this.capabilities.fan.fanLevels)
	...
	acAccessory.prototype.setDryRotationSpeed = function (speed, callback) {
		...
		speed = HKtoFanLevel(state, this.capabilities.dry.fanLevels)
	...
	acAccessory.prototype.setSyncState = function (callback) {
		this.log(this.name + ' - Syncing AC State => Setting ' + (this.state.acState.on ? 'OFF' : 'ON') + 'state without sending commands')
		try {
			const syncResponse = await sensibo.syncDeviceState(this.id, !this.state.acState.on)
			if (syncResponse.status !== 'success')
				throw new Error(syncResponse.result.failureReason)

			this.updateHomeKit(syncResponse.result.acState)
			...
			console.log('Could not Sync AC State for', this.name)
			if (this.debug)
				this.log(err)
			callback(err)
		}
	...
	acAccessory.prototype.setNewState = function (key, value) {
		this.setCommands = this.setCommands[key] = value

		if (!this.setProcessing) {
			this.setProcessing = true
			setTimeout(() => {
				const commands = this.setCommands
				let newState = this.state.acState

				if (('acPower' in commands && commands['acPower'] === false && (this.state.acState.mode === 'cool' || this.state.acState.mode === 'heat' || this.state.acState.mode === 'auto')) 
				|| ('fanPower' in commands && commands['fanPower'] === false && this.state.acState.mode === 'fan') 
				|| ('dryPower' in commands && commands['dryPower'] === false && this.state.acState.mode === 'dry')) {
					this.log('Turning OFF ' + this.name)
					newState.on = false
				} else {
					if ('acMode' in commands){
						if (this.state.last[commands['acMode']])
							newState = this.state.last[commands['acMode']]
						newState.on = true
						...
						if (this.state.last.mode && this.state.last[this.state.last.mode])
							newState = this.state.last[this.state.last.mode]
						else
				...
				switch (newState.mode) {
					...
					default:
						...
						if ('acFanLevel' in commands)
							newState.swing = commands['acFanLevel']
						break
				...
				if (newState.mode !== 'fan' && newState !== 'dry')
	...
	acAccessory.prototype.updateHomeKit = function (acState, measurements) {
		if (measurements) {
			if (this.debug) {
			...
			this.HeaterCoolerService.getCharacteristic(Characteristic.CurrentTemperature).updateValue(this.temperatureUnit === 'C' ? measurements.temperature : toCelsius(measurements.temperature))
		...
		// if ac is on
			...
			if (acState.mode === 'fan') {
				...
				if (this.capabilities.fan && !this.disableFan) {
					...
					if (acState.swing)
						this.FanService.getCharacteristic(Characteristic.SwingMode).updateValue(swing)
			...
			// if mode is dry
				...
				if (this.capabilities.dry && !this.disableDry) {
					...
					if (acState.swing)
						this.DryService.getCharacteristic(Characteristic.SwingMode).updateValue(swing)
			...
			// if mode is cool, heat or auto
				...
				if (acState.mode === 'cool') {
					...
					this.HeaterCoolerService.getCharacteristic(Characteristic.TargetHeaterCoolerState).updateValue(Characteristic.TargetHeaterCoolerState.COOLING)
					this.HeaterCoolerService.getCharacteristic(Characteristic.HeatingThresholdTemperature).updateValue(targetTemperature)
				} else if (acState.mode === 'auto') {
				...
				if (acState.swing)
					this.HeaterCoolerService.getCharacteristic(Characteristic.SwingMode).updateValue(swing)
	...
	function occupancyAccessory(config, state) {
		this.name = config.name
		this.log = config.log
		this.debug = config.debug
		this.state = state
		this.occupied = state.occupancy === 'home'
	...
	occupancySensor.prototype.getServices = function () {
		...
		this.informationService = new Service.AccessoryInformation()
			.setCharacteristic(Characteristic.Manufacturer, 'Sensibo Occupancy')
			.setCharacteristic(Characteristic.Model, this.model)
			.setCharacteristic(Characteristic.SerialNumber, this.id)
	...
	occupancySensor.prototype.getStatus = function (callback) {
		if (this.state.occupancy === 'home')
			this.log('Someone is Home!')
		else
			this.log('No One is Home!')
		callback(null, this.state.occupancy === 'home')
	}

	occupancySensor.prototype.updateHomeKit = function (state) {
		if (state.occupancy === 'home')
	...
	return {acAccessory, occupancyAccessory}
}
