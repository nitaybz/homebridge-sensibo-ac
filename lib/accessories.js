...
module.exports = function (Accessory, Service, Characteristic, HomebridgeAPI, uuid, FakeGatoHistoryService) {
	...
	function acAccessory(config) {
		...
		this.externalHumiditySensor = config.externalHumiditySensor
	...
	acAccessory.prototype.getServices = function () {
      ...
      || (this.capabilities.fanLevels && this.capabilities.fanLevels && this.capabilities.heat.fanLevels.length)) {
		...
		if (this.capabilities.dry && !this.disableDry) {
			...
			if (this.capabilities.dry.fanLevels && this.capabilities.dry.fanLevels.length) {
		...
		if (this.externalHumiditySensor) {
			if (this.debug)
				this.log('Setting Climate React Button for', this.name)
			this.HumiditySensor = new Service.HumiditySensor(this.name + ' Humidity', 'externalHumidity')
	...
	acAccessory.prototype.getCelsiusTargetTemperature = function (temp) {
		if (this.usesFahrenheit()) {
			if (temp)
		...
		if (!this.setProcessing) {
			...
			setTimeout(() => {
				...
				|| ('dryPower' in commands && commands['dryPower'] === false && this.state.acState.mode === 'dry' && !('dryFanLevel' in commands))) {
					...
					} else if (('acPower' in commands && commands['acPower'] === true) || 'coolTemp' in commands || 'heatTemp' in commands || 'acSwing' in commands || 'acFanLevel' in commands){
				...
				if (JSON.stringify(this.state.acState) !== JSON.stringify(newState)) {
					...
					newState.temperatureUnit = this.temperatureUnit
	...
	acAccessory.prototype.updateHomeKit = function (acState, measurements, smartMode) {
		if (measurements) {
			...
			if (this.externalHumiditySensor)
				this.HumiditySensor.getCharacteristic(Characteristic.CurrentRelativeHumidity).updateValue(measurements.humidity)

			if (this.capabilities.dry && !this.disableDry && !this.setProcessing) {
				this.DryService.getCharacteristic(Characteristic.CurrentRelativeHumidity).updateValue(measurements.humidity)
			...
			const newEntry = {
				...
				currentTemp: measurements.temperature,
				setTemp: acState ? this.getCelsiusTargetTemperature(acState.targetTemperature) : this.getCelsiusTargetTemperature(),
				status: acState ? (acState.on ? 1 : 0) : (this.state.acState.on ? 1 : 0)
			}

			// log new state
			this.loggingService && this.loggingService.addEntry(newEntry)
		...
		if (acState && !this.setProcessing) {
			...
			// if ac is on
				...
				if (acState.targetTemperature)
					targetTemperature = this.getCelsiusTargetTemperature(acState.targetTemperature)
