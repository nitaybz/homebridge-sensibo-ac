let Accessory, Service, Characteristic, uuid, SensiboAccessory
...
module.exports = function (Accessory, Service, Characteristic, uuid) {
	...
	acAccessory.prototype.updateHomeKit = function (acState, measurements) {
		...
		if (acState && !this.setProcessing) {
			...
			this.state.acState = acState
			this.state.last[acState.mode] = acState
			if (acState.mode !== 'fan' && acState.mode !== 'dry')
				this.state.last.mode = acState.mode

		}


		if (this.debug && !this.setProcessing) {
			this.log(`Finished updating state in HomeKit for ${this.id}:`)
			this.log(acState)
			this.log(measurements)
		}
	}
	...
	function occupancySensor(config, state) {
		this.name = config.name
		this.log = config.log
		this.debug = config.debug
		this.state = state
